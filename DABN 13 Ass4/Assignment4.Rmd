...
Z <- Z.matrix.1d
pcr.coefs.1d   <- solve(t(Z)%*%Z)%*%t(Z)%*%y.1a
pcr.pred.1d    <- Z%*%pcr.coefs.1d
...
pcr.pred.1e    <- as.vector(predict(pcr.fit.1e, data = data.1e, ncomp = 2))
...
Our next task is to extract the specific training and holdout fold for the $j$-th iteration of the CV algorithm. First, collect your predictors and then your outcome in a data frame `data.2b`, precisely as in task 1e. However, now allocate the generic name `y` to the column containing the outcome. Then, create two objects `data.train.2b` and `data.holdout.2b` that contain these folds. Use the objects created in the last task above for indexing and refer generally to their $j$-th row.   
...
Mmax <- 5
pcr.fit.2c      <- pcr(y ~ ., data = data.train.2b, ncomp = Mmax, scale = TRUE, center = TRUE)
pcr.pred.2c     <- predict(pcr.fit.2c, newdata = data.holdout.2b, ncomp = 1:Mmax)
...
num.itr       <- nrow(train.2a)
CVloss.all.2d <- matrix(NA, nrow = num.itr, ncol = Mmax)

for (j in 1:num.itr) {
  for(m in 1:Mmax){
    CVloss.all.2d[j,m] <- sum((data.holdout.2b$y[j] - pcr.pred.2c[,,m])^2)
  }
 }
...
Lastly, assuming that all `num.itr` rows of `CVloss.all` were filled instead of simply the very first such row, create a vector `CVloss.mean.2e` that takes averages of the elements in every column of `CVloss.all`. Use the `apply()` command to do this. To simplify assignment correction to us, please copy and paste the specific command that you use into the string variable ` my.apply.command.2e`.

``` {r }

CVloss.mean.2e  <- apply(X=CVloss.all.2d,MARGIN=2,FUN=mean)
my.apply.command.2e <- "mean"
...
num.itr       <- nrow(train)
CVloss.all.2f <- matrix(, nrow = num.itr, ncol = Mmax)
    
for (j in 1:num.itr) {
data.train   <- data[train[j,1]:train[j,2],]
data.holdout <- data[holdout[j,1]:holdout[j,2],]
pcr.fit.2f      <- pcr(y~ ., data = data.train, ncomp = Mmax, scale = TRUE, center = TRUE)
pcr.pred.2f     <- predict(pcr.fit.2f, newdata = data.holdout, ncomp = 1:Mmax)
      
for (m in 1:Mmax) {
  CVloss.all.2f[j,m] <- sum((data.holdout$y[1] - pcr.pred.2f[,,m])^2)
}

 }

  CVloss.mean  <- apply(X=CVloss.all.2f,MARGIN=2,FUN=mean)
...
data.train   <- data[train[j,1]:train[j,2],]
data.holdout <- data[holdout[j,1]:holdout[j,2],]
plsr.fit.2f      <- plsr(y~ ., data = data.train, ncomp = Mmax, scale = TRUE, center = TRUE)
plsr.pred.2f     <- predict(plsr.fit.2f, newdata = data.holdout, ncomp = 1:Mmax)
      
for (m in 1:Mmax) {
  CVloss.all.2f[j,m] <- sum((data.holdout$y[1] - plsr.pred.2f[,,m])^2)
}
...
which.nums.chosen.2h <-"For PCR, the number 15 is chosen and for PSL, the number 5 is chosen."
...
n.init.3a <- 450
train.3a  <- cbind(1:(n.obs.3a-n.init.3a), n.init.3a:(n.obs.3a-1)) 
test.3a   <- cbind(n.init.3a:(n.obs.3a-1)+1, n.init.3a:(n.obs.3a-1)+1) 
...
why.expand.train.set3b = "It makes sense to use t-1 data in the training data, because it is the most recent and therefore the most accurate when using in the training data. Especially since it's financial data where the market is very volatile."
```
...
pcr.pred.3c <- matrix(, nrow = dim(test.3a)[1], ncol = 1)
pls.pred.3c <- matrix(, nrow = dim(test.3a)[1], ncol = 1)
...
``` {r }
 ...
 # Define cross-validation samples
   ...
   pcr.fit.3d          <- pcr(y~., ncomp = pcr.mhat, data = data.3a.train, scale = TRUE, center = TRUE)
   pcr.pred.3c[itr]    <- predict(pcr.fit.3d, newdata = data.3a.test, ncomp = pcr.mhat)
 
   pls.fit.3d          <- plsr(y~., ncomp = pls.mhat, data = data.3a.train, scale = TRUE, center = TRUE)
