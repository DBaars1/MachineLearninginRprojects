...
task1a.linkfunction <- "log, to transform the non-linear relationship to linear form."
```

### task 1b)

In the lecture $AIC$ was used to show highlight improvement of fit for GLM moldes. 
...
task1b.does.backward.select.best.model <- "Backward selection does not check all possible combinations, therefore is it not the best possible model. However, it is efficient and it helps us improve the model. In the step model we can clearly see that the AIC is reduced compared to the full model."
...
```{r}
library(countreg)
rootogram(model.poisson, style = 'standing')
task1c.describe.rootogram <- "The rootogram shows deviations, because the curve representing expected frequencies slightly differs the histogram observed frequencies. The pattern reflect an amount of overdispersion that is not captured by the fitted Poisson distribution."
task1c.poisson.fit.rootogram <- "The model does not fit well, since the observations do not match the line."
```
...
task2a.rootogram.model.fit.data <- "The rootogram suggest the nb model fits the data better, because the curve has less deviations with the histogram observed frequencies." 
task2a.AIC_compare <- "The negative binomial fits better because it has a lower AIC (21216) compared to the Poisson regression (30165)."
...
task2b.coeff_res <- "In the Poisson model, the coefficients are significant, and p-values are low across the coefficients. In the negative binomial, almost all variables have a worse significance, because almost all p-values are higher (except totchr) in the negative binomial model, therefore has a has a reduced certainty.'
"
```
...
```{r}
task3a.what.is.zero.prob <- "pi^{zero}(0;theta,mu,pi) = pi^{zero} + (1-pi^{zero})p(0; mu, theta)"
```
...
```{r}
task3b.negbin.docvis.zero <- " No, the model has 20 counts at the zero count bin, and the model predicted 17, meaning there is a situation of underpredicting. The corresponding negative binomial model does not correctly predict the number of zero counts."
```
...
```{r}
model.zero.negbin <- zeroinfl(formula = docvis ~.|., data = data, dist = "negbin")
```

### task 3d)
Now do backward selections again and create the rootogram. Does the model fit the data better?
What about the AIC of the final model, which model is best according to $AIC$ and what of all things starting from the full Poisson regression model gave the largest improvement in $AIC$?
```{r}
...
task3d.AIC     <- "The model fits the data better, since the histogram observed frequencies fit better the predicted line, including the zero-bin bar. Also, the model is better since it has an improved AIC (21031) compared to Poisson (30165) and negative binomial (21216). Out of the models, the backward step AIC of the zero negative binomial model gave the best improvement."
```



## Part 4: multinomial regression and grouped lasso
...
```{r}
set.seed(12345)
cvfit <- cv.glmnet(X, y, family = 'multinomial', type.multinomial = 'grouped', intercept=F)
coef4a <- coef(cvfit,s="lambda.1se")$"within a year"
task4a.which.effects.most <- "The alcohol consumption within a year seems to increase the probability to have used drus within a year the most."
```
...
y.pred.response <- predict(object = cvfit, newx=X.test, s = "lambda.1se", type = 'response')
y.pred.class <- predict(object = cvfit, newx=X.test, s = "lambda.1se", type = 'class')
y.pred.link <- predict(object = cvfit, newx=X.test, s = "lambda.1se", type = 'link')
task4b.predict.response.is <- "Response gives the fitted probabilities"
task4b.predict.class.is    <- "Class produces the class label corresponding to the maximum probability"
task4b.predict.link.is     <- "Link gives the linear predictors"
task4b.predict.connection.response.and.class    <- " We work with a multinomial model, because we have more then two classes. Type=response gives the fitted probabilities, whereby type=class gives the label of the class with the highest probability of each observation."
```
...
X.test <- model.matrix(drugs.usage ~ 0 + ., data = drug.data.test)
y.test <- drug.data.test$drugs.usage
y.pred <- predict(object = cvfit, newx=X.test, s = "lambda.1se", type = 'class')
